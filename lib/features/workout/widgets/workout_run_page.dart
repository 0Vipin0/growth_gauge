  ...
  Widget build(BuildContext context) {
    ...
    if (provider.template == null) {
      return Scaffold(
          ...
          body: const Center(child: Text('No workout running')));
    }
        ...
        child: Column(
          ...
          children: [
            Row(
              ...
              children: [
                Text(
                    'Step ${provider.currentIndex + 1}/${provider.template!.steps.length}',
            ...
            Text(step?['name']?.toString() ?? 'Untitled',
                style: Theme.of(context).textTheme.headlineSmall),
            const SizedBox(height: 12),
            if (step != null && step.containsKey('duration'))
              Center(
                  child: Text('${provider.remaining ?? 0}s',
                      style: Theme.of(context).textTheme.headlineSmall)),
            if (step != null && step.containsKey('reps'))
              Center(
                  child: Text('${step['reps']} reps',
                      style: Theme.of(context).textTheme.headlineSmall)),
            if (step != null && step.containsKey('sets'))
              Column(children: [
                Text('Set ${provider.currentSetIndex + 1}/${step['sets']}',
                    style: Theme.of(context).textTheme.titleMedium),
                const SizedBox(height: 8),
                ElevatedButton(
